...
from helper.utils.response.response_format import bad_request_response, success_response
...
class Paystack:
    ...
    def get_header():
        headers = {
            "Authorization": f"Bearer sk_test_c46b63f976e8b0de9bcda7575240c65e32e2c0bd",
            "Content-Type": "application/json"
        }
        return headers

    @staticmethod
    def charge_user(request, application, *args, **kwargs):
        transaction=None
        try:
            amount = 5000  # Example amount (in kobo)
            user = kwargs.get("auth_user")
            redirect_url = f"{request.data.get('redirect_url')}&payment_type=PAYSTACK"
    ...
    @staticmethod
    def validate_payment(transaction_id,redirect_url):
        try:
                        ...
                        if transaction.payment_status != "SUCCESS":
                            ...
                            if payment_type == "NIN":
                                try:
                                    ...
                                    # confirm the appointment with printscan
                                    # capturing_order_comfirmed
    ...
    def process_webhook(payload):
        try:
            ...
            return bad_request_response(message='Failed to process webhook.')
